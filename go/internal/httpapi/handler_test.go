...
func TestEmbeddedUIAndSecurityHeadersAreServedWithoutFrontendBusinessRules(t *testing.T) {
	...
	for _, required := range []string{
		...
		"WORKFLOW_REVIEWER_ASSIGNMENT_REQUIRED", "renderTimeline", "rememberError", "setBackgroundWorking", `requestJSON("/v1/workspace-status")`, "最初のAIチームを確認",
		...
		"renderInFlight", "storedPendingCommand", "依頼内容を確認して、進め方を整理しています", "Makerの成果物作成、QA担当のReview、必要なRevisionを順番に進めます。",
		"liveStatusNode", "pendingInteractionNodes", "renderMarkdown", "safeMarkdownLinkHref", "deliverableViewerNode",
		"この画面を閉じても処理はバックグラウンドで続きます。",
		"commandProviderFailure", "task?.execution?.provider_failure",
		"restoreDurableFailure", "record.failure.details", "前回のCommandを完了できませんでした",
		"providerSetupError", "Claude APIキーをmacOS Keychainへ保存できませんでした", "Keychainへ保存",
