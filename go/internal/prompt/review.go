...
func (builder Builder) BuildReview(ctx context.Context, input review.PromptInput) (worker.Prompt, error) {
	...
	reviewRules := strings.Join([]string{
		...
		"",
		"## 出力形式（厳守）",
		"1. まず人間向けのMarkdownでレビュー内容を記述してください。",
		"2. その直後に、開始マーカー REVIEW_RESULT_JSON_START を正確に1回だけ出力してください。",
		"3. 開始マーカーの直後から終了マーカーの直前までは、有効なJSONオブジェクト1つだけを出力してください。前後の説明文、Markdown code fence（```）、コメント、余分な空行やテキストを一切含めないでください。",
		"4. 終了マーカー REVIEW_RESULT_JSON_END を正確に1回だけ出力してください。この2つのマーカー以外の場所にマーカー文字列を出力しないでください。",
		"5. JSONのtop-level fieldはverdictとissuesの2つだけにしてください。それ以外のfieldを追加しないでください。",
		`6. verdictの値は、次の2つの文字列のどちらか一方をそのまま使用してください（言い換え・翻訳・組み合わせは禁止）: "Approve" または "Request Changes"`,
		"7. issuesは常にJSON配列にしてください（指摘がない場合は空配列[]、verdictがRequest Changesの場合は1件以上）。",
		"8. issuesの各要素は、category・severity・description・suggested_actionの4 fieldだけを持つオブジェクトにしてください。",
		"9. categoryの値は date, format, requirements, context, todo, other のいずれか1つの文字列にしてください。",
		"10. severityの値は high, medium, low のいずれか1つの文字列にしてください。",
		"11. descriptionとsuggested_actionは空文字列にせず、具体的な内容を記述してください。",
		"",
		"## 出力例（構造の見本です。この値をそのままコピーせず、あなた自身の判断内容に差し替えてください）",
		"REVIEW_RESULT_JSON_START",
		`{"verdict":"Request Changes","issues":[{"category":"requirements","severity":"medium","description":"要件Xへの言及がありません。","suggested_action":"要件Xについて追記してください。"}]}`,
