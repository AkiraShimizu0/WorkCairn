...
func (builder Builder) BuildReview(ctx context.Context, input review.PromptInput) (worker.Prompt, error) {
	...
	reviewRules := strings.Join([]string{
		...
		"",
		"## 必須マーカー（例外なし）",
		"REVIEW_RESULT_JSON_START と REVIEW_RESULT_JSON_END は、成果物の長さ、verdict、issuesの有無にかかわらず、すべての応答で必ず出力してください。",
		"成果物が短い、レビューが簡潔、Approveで指摘なし、のいずれの場合も省略しないでください。",
		"この2つのマーカーまたはJSONを省略した応答はWorkCairnで処理できません。",
		"出力順序は、人間向けMarkdown → REVIEW_RESULT_JSON_START（正確に1回） → JSONオブジェクト1つだけ → REVIEW_RESULT_JSON_END（正確に1回）を厳守してください。",
		"マーカーの間にはJSON以外のテキスト、Markdown code fence（```）、コメントを一切含めないでください。",
		"",
		"## 出力形式",
		"1. JSONのtop-level fieldはverdictとissuesの2つだけにしてください。それ以外のfieldを追加しないでください。",
