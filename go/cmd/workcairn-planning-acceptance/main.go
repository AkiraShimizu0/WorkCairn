...
func run(ctx context.Context, args []string, stdout, stderr io.Writer, deps dependencies) int {
	flags := flag.NewFlagSet("workcairn-planning-acceptance", flag.ContinueOnError)
	flags.SetOutput(stderr)
	provider := flags.String("provider", planningacceptance.ProviderFakeGood, "fake-good, fake-bad, or claude")
	execute := flags.Bool("execute", false, "perform the bounded acceptance run; omitted means dry-run (claude only -- fake-good/fake-bad always execute)")
	artifactPath := flags.String("artifact-path", "", "optional: write the Review Artifact (canonical Normalized Plan + safe metadata) to this file, outside the Git working tree and any real Vault; omitted writes nothing")
	if err := flags.Parse(args); err != nil || flags.NArg() != 0 {
		return 2
	}
	config := planningacceptance.Config{Provider: strings.TrimSpace(*provider), Execute: *execute, ArtifactPath: strings.TrimSpace(*artifactPath)}
	if config.Provider == planningacceptance.ProviderClaude && config.Execute {
	...
	result, err := planningacceptance.Run(ctx, config)
	if encodeErr := json.NewEncoder(stdout).Encode(result); encodeErr != nil {
		return 1
	}
	if err != nil {
		fmt.Fprintln(stderr, "Planning acceptance did not complete; inspect the safe JSON report")
		return 1
	}
	return 0
}
