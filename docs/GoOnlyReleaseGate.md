...
| Interaction Session | `interaction-start-*`、`interaction-plan-generate`、`interaction-answer`、`interaction-plan-apply`、`interaction-workflow-*`、`interaction-action-*` | OK | request／plan／Workflow／Action digest承認、質問block、既存Workflow／Action child composition、Session CAS、Ledger replay、temporary Vault／Mock Provider E2E |
