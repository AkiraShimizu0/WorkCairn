...
- [x] 全Go test、vet、gofmtが成功する（`v1.0.0-beta.1` tag対象commitでの確認時点）。raceは`v1-release-gate`実行では成功。その後のbackend専用機能追加（M-RECOVERY-3系、tag対象commitより後のfeature HEAD）で、`make v1-release-gate`を1回実行したところ、`make public-beta-smoke`は全項目PASS、同Gate内のbuild、release build matrix、raceなしの通常全Go testはPASSしたものの、続く`go test -race -count=1 ./...` componentで`TestRunParallelProviderCallBudgetStopsBeforeExceedingLimit`が1件非決定的に失敗し、Gateはそこで停止しました（vet、gofmt、script check等の後続componentは未実行）。既存方針どおりretryはせず、focused correction（M-RECOVERY-3D）へ分離しました。実際にM-RECOVERY-3Cで失敗したのはこの1 testだけです。`TestRunParallelBudgetPartialFailurePreservesOtherBranches`は同じ欠陥のあるgeneric error assertionを持っていたため、M-RECOVERY-3Cでは失敗していませんが、M-RECOVERY-3Dで同一contract defectの予防修正として同時に直しました。原因は、並行`reserveProviderCallBudget`の2つの停止経路（BudgetPolicy read／tracker atomic reservation）のうち一方の経路でしか成立しない汎用`ErrBudgetExceeded`をtest assertionが必須化していたtest contract defectであり、production側のBudgetGuard／atomic reservationによる上限強制に不具合は確認されていません。assertionを両経路で共通に成立する`stage=="budget"`かつ`errors.Is(err, ErrProviderCallBudgetExceeded)`へ同期しました。M-RECOVERY-3Dでは、対象2 testの通常実行、`go test -race -count=50`のfocused stress command、full `go test -count=1 ./...`、full `go test -race -count=1 ./...`、`go vet ./...`、`git diff --check`がいずれもPASSしたことを確認済みですが、`make v1-release-gate`自体はまだfreshに再実行していません。この現在のfeature HEADに対するAutomated Gates Greenの確定は、修正commit後にfresh `make v1-release-gate`を実行して確認する必要があります。
