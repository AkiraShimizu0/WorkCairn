...
## Known gap: Planning Output Completeness

`worker.RunResult.Validate()` does not check `StopReason` at all, and ADR-0058's `OUTPUT_INCOMPLETE` classification is wired only into `ExecutionService.Execute()` (Task execution) — never into `service.CEOPlanService.Generate()`. A real, truncated Planning generation would most likely fail `ParseIntent`'s strict JSON decode and surface as an ordinary `json_decode_failed`/`ceo_plan_intent` failure, indistinguishable from "the LLM returned garbage." This is a confirmed, real architecture gap (Phase P), **not fixed by this Checkpoint** — candidate for a future `PHASE R — Planning Output Completeness Policy`, ideally informed by a real Planning run the same way ADR-0058 itself was.
