...
| Go PromptBuilder | 構造化された会社・社員・日時・Project・Task Contextから通常Task用Promptを決定的に構築する。ADR-0056により、直接依存Taskのcanonical Deliverableをprovenance付きの信頼されないUser Contextとして順序どおり追加し、UTF-8安全な固定上限で決定的に切り詰める。依存なしのPromptは既存goldenとbyte-for-byte互換 |
