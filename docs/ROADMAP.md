...
M-RECOVERY-6Aでは、Guided Recovery Inspectionのrecoverable FindingからHumanの明示clickでだけPreviewを取得するLocal Web UIを実装しました。Finding単位のReason入力、strict preflight、responseのclosed validation、ADR-0073由来のsingle-flight／stale-response ownership、read-only terminal表示を追加し、Apply／自動repair／retryは追加していません。M-RECOVERY-6BではProjectを含むPreview ownershipとmobile成功UIの実寸evidenceをfocused correctionし、Codex focused reviewをP0〜P3なしで完了しました。M-RECOVERY-6B.3でUIとADR-0074の`Accepted`化を単一commitにまとめました。M-RECOVERY-6Cではfeature HEADに対する`make public-beta-smoke`と`make v1-release-gate`がPASSし、`make check-ui-full`は160 passed／1 known skip／0 failed／0 retriesで完了しました。known flakeとtracked generated diffはありません。M-RECOVERY-6Dで同commitをGitHub mainへ通常のfast-forwardでpushし、backend／UI／review／Gate／main反映まで完了しました。
...
## Completed — Complete Task Recovery Apply
...
- focused review GOに基づきADR-0075／ADR-0076を`Accepted`へ同期し、Final Automated Gates、単一feature commit、GitHub main反映まで完了した。tag、Release、asset、candidate、Acceptance evidenceは変更していない

## Next 2 — Bounded Provider Acceptance Profile（実装済み・Codex GO・ADR Accepted・commit済み。Public Betaは引き続きNO-GO）

Codexのfocused reviewが、既存のKeychain-only daemon／HTTP経路では「Plan生成1回・Task1件・Review1回・Provider call合計最大3回・clarification／failure／timeout／Request Changesで確実に停止・Revision／Recovery／retry／fallbackなし」というHuman Acceptance実行中の有界性を構造的に保証できないと診断しました（Revision作成自体がProvider呼び出しを1件も伴わないため、既存BudgetGuard v1だけではRevision作成を止められないことが中心的な理由）。PHASE PB-3an.1で、この診断へ応える既定OFF・additiveな`bounded_acceptance` profileの契約を[ADR-0072](adr/ADR-0072-bounded-provider-acceptance-profile.md)（Status: **Accepted**、PB-3an.2eのCodex implementation reviewがGOを出し、PB-3an.2fでAcceptedへ変更・31ファイルcommit済み）として提案しました。

- 標準profile（`autonomy.NewStandard`）の挙動・serialized出力は無変更とする設計
- Keychain経由の既存credential解決、Provider endpoint、model routingは無変更。operator CLIへのcredential export、環境変数転記は不採用
- Plan生成（Interaction Session scope、process再起動を跨ぐdurable reservation）とTask実行＋Review（Reviewed Workflow scope、既存BudgetGuard `MaxProviderCalls=2`を再利用）を分けて会計し、合計3回に抑える設計（PB-3an.2で実装、PB-3an.2eのCodex implementation review GOにより実装・testの妥当性を確認済み）
- 既存の未使用`autonomy.Contract.Revision` Permission fieldへ`forbidden`を許容する拡張と、`service.reviser.Execute`呼び出しを単一のpolicy-aware境界へ集約する設計により、Request Changes後のRevision作成をReview保存後・Revision作成前で停止することを提案
- Request Changesの停止は既存のTyped FailureEnvelope伝播（[ADR-0041](adr/ADR-0041-typed-failure-envelope-propagation.md)）を再利用し、技術障害・成功完了のいずれとも異なる専用Code`REVIEWED_WORKFLOW_BOUNDED_STOP`／`RecoveryRequired: false`として表現する設計
- 同一承認対象への並行二重承認は、`{SessionID, ExpectedVersion, PlanDigest, Profile}`という閉じた型付き入力から決定的に導出するdurable reservationで防止する設計（bounded profile限定、標準profileの既存挙動は変更しない）
- 一般daemonがexact allow-listする`interaction.*` operation全数を確認し、`interaction.plan.approve_and_execute`以外の実行系standalone入口（`interaction.plan.apply`／`interaction.workflow.execute`／`interaction.workflow.recover_revision`）もbounded Sessionに対してProcess層でeffectより前に拒否する設計とし、UI非表示を安全性の根拠にしない

PHASE PB-3an.1aのCodex focused reviewでP1 5件・P2 2件、PHASE PB-3an.1cで残ったP1 4件・P2 3件、PHASE PB-3an.1eで残ったP1 1件（`interaction.Record.Validate()`自身にProfileのclosed-value guardが欠けており、unknown Profile値がfail-openし得るという指摘）を指摘され、PHASE PB-3an.1b／PB-3an.1d／PB-3an.1fで同じ3文書（ADR-0072、本ROADMAP、`docs/Architecture.md`）だけへfocused correctionを重ねました。これまでに指摘された設計段階の技術的なP1／P2はすべて本文でsource根拠付きに解消済みです。

PHASE PB-3an.2で、この確定した設計をそのまま実装契約として、production vertical sliceを実装しました。`go/internal/autonomy`（`NewBoundedAcceptance`、`Validate()`のRevision緩和）、`go/internal/interaction`（`Profile`型、`TurnPlanGenerationReserved`、`NewWithProfile`、`Validate()`のclosed-value guard、`ValidateTransition`のProfile不変性、profile-aware `Next()`）、`go/internal/commandledger`（`DeriveApprovalReservationID`）、`go/internal/process`（`interaction.go`／`interaction_workflow.go`／`interaction_recover_revision.go`／`interaction_approve_and_execute.go`／`reviewed_workflow.go`への1 Task preflight・approval reservation・全standalone入口のfail-closed化・bounded stop envelope）、`go/internal/service`（`Run`／`RunParallel`／`ResumeRevision`への`revisionPermission`配線、`executeRevisionIfAllowed`）、`go/internal/commandcontract`／`go/internal/httpapi`（`execution_profile`の受理・検証・橋渡し）、`go/internal/httpapi/web`（既定OFFのtoggle、Plan承認画面のread-only表示、bounded stop表示copy）のすべてに実装とtestを追加し、gofmt／go vet／targeted `go test`（対象packageは`-race`込み）・全package`go test ./...`をこのCheckpoint内で実行し、いずれもPASSしました（新規追加testを含む）。

実装作業自体が、ADR本文の設計だけでは見えていなかった実バグ2件（Interaction Workflow層の別関数`workflowFailure`がReviewed Workflow層の`RecoveryRequired`修正を無条件上書きしていた点、Revision禁止判定の前にCommand IDが導出され幽霊`RevisionCommandID`がTask evidenceへ残っていた点）をtestで検出し、同じCheckpoint内で修正しました。

PHASE PB-3an.2aのCodex focused reviewは、この実装自体からP1 4件・P2 2件・P3 1件を指摘しました：(1) `Record.Validate()`／`RecordPlan`がbounded Sessionの`TurnPlanGenerated`とその直前の`TurnPlanGenerationReserved`の対応を検証しておらず、reservation invariantがドメイン層で強制されていなかった、(2) `go/internal/httpapi/handler.go`の`mapCommandError`がHTTP最上位の`recovery_required`を`Envelope.RecoveryRequired`ではなく無条件`Partial`から計算しており、`web/app.js`のpolling／reload復元ロジックも独立に同種のバグ（`state === "partial_failure"`だけからの再生成）を持っていた、(3) 「並行承認」testが実際には2回の逐次呼び出しに過ぎなかった、(4) `DeriveApprovalReservationID`が`bounded_acceptance`以外の空文字列や任意文字列も受理していた、(5)〜(7) いくつかの境界test（standard digest互換、Store decode/encode拒否、HTTP伝播、timeout変種、Request Changes後の直接evidence）が不足していた。PHASE PB-3an.2bでこれらすべてをfocused correctionし（`Record.Validate()`／`RecordPlan`へのinvariant追加、Go／JS両方のRecoveryRequired投影の統一、barrier channelによる実際の2 goroutine競合test（`go test -race`20回連続PASS）への置き換え、`DeriveApprovalReservationID`のProfile closed化、不足testの追加）、さらに一般browser UIの実装をfixture駆動のMock Providerと実際のdaemon binaryを使った新規Playwright spec（`tests/browser/bounded-acceptance.spec.mjs`、Request Changes停止経路とApprove完了経路の2 scenario）でEnd-to-End確認しました。詳細はADR-0072「Implementation Findings Resolved」。

PHASE PB-3an.2cのCodex focused reviewは、実装そのものではなくtest evidenceの厳密さについてP1 2件を指摘しました：(1) `TestBoundedApproveAndExecuteConcurrentApprovalRaceOnlyOneWins`がbarrier channelで2 goroutineを同時解放してはいたが、winnerとloserが実際に時間的に重なって競合したことを決定的に証明する仕組みがなく、たまたま正しい終端状態になっただけの可能性を排除できていなかった、(2) `tests/browser/bounded-acceptance.spec.mjs`のRecovery button非存在確認が緩い正規表現（`/追加の指示/`・`/修正を依頼/`）に留まり、Provider呼び出し回数・順序の確認もfixtureが3件のresponseを持つことへの間接的な依存に留まり、standard profileでの`execution_profile`欠落もUI文言経由の間接確認のみだった。PHASE PB-3an.2dで、**production codeを一切変更せず**test強化のみでこれらを解消しました：並行承認testへtest専用のblocking fake（`raceApproveAndExecuteMockServer`）を追加し、winnerがreservation `consumed`後・Task execution Provider callへ到達した時点でpauseさせ、loserが独立にterminal状態へ到達したことを確認してから初めてwinnerを解放する構造へ書き換え（`go test -race`20回連続PASS）、Browser E2Eは実際の製品button文字列（「この指摘を踏まえて修正を続ける」／「必要な部分だけ続ける」）・composerの`data-mode="idle"`／readonly・`#details-panel`のReview evidence永続表示を停止直後とreload後の両方で直接assertし、`environment.provider.calls`配列の直接検査でProvider呼び出しの回数・順序（Plan=structured, Task=unstructured, Review=structured）をexact matchで確認し、standard profileでの`execution_profile`欠落もnetwork capture経由で直接確認する新規testへ強化しました。この対応でproduction defectは1件も新たに見つかっていません。詳細はADR-0072「Implementation Findings Resolved」（項目8）および「Test matrix」（PB-3an.2dで強化したtest）。

PHASE PB-3an.2eのCodex implementation reviewはGOを出し（31ファイルcommit-ready、ADR Accepted昇格可能と判定）、PHASE PB-3an.2fで、この判定に基づき[ADR-0072](adr/ADR-0072-bounded-provider-acceptance-profile.md)のStatusを`Proposed`から`Accepted`へ変更し、本ROADMAPおよび`docs/Architecture.md`を同期した上で、上記31ファイルを単一commitとして記録しました（実装・test内容自体はPB-3an.2b／PB-3an.2dから無変更、status-only docs同期とcommitのみ）。**Public Betaは引き続きNO-GOです**——本Checkpointは実Providerでの成功確認・PB-3ag／PB-3jのProvider failure根本原因修正のいずれも主張しません。次のステップは、この新しいHEADに対するFinal Automated Gates（`make check-ui-full`／`make public-beta-smoke`／`make v1-release-gate`）の実行、push、`v1.0.0-beta.1`タグの再作成、signed candidateの再生成、Human Provider／Upgrade Acceptanceの実施です。現行の旧signed candidateおよび旧Acceptance evidence（旧HEAD `6d9a4d0f3002d08087b42fe6afeaf39c16f63d73`由来）は、新HEADのRelease evidenceとして再利用しません。

**実装・testが揃ったことと、Codexの実装reviewが完了していることは別の問いでした。** PB-3an.2〜PB-3an.2dの間、Codex implementation reviewがGOを出すまではADR-0072のStatusを`Proposed`から動かさず、`Accepted`への変更はCodexのGO後、同じCheckpoint系列内でのみ行うという契約を維持しました。PB-3an.2eでそのGOを得たため、PB-3an.2fでStatusを`Accepted`へ変更し、31ファイルを単一commitとして記録しました（詳細は上のPHASE PB-3an.2fパラグラフ）。この機能は、PB-3ag／PB-3jで観測されたProvider failure根本原因の修正でも、実Providerでの成功確認でも、Public Beta GOへの近接を示すものでもありません。**Public Betaは本Checkpoint終了時点でも引き続きNO-GOのままです。** 現行のsigned candidate（Manual macOS Signed Release Procedureに基づく既存candidate、Acceptance evidence）は旧HEAD由来のままであり、新HEADに対する変更・再利用可否の判断・Final Automated Gates・push・tag再作成・candidate再生成・Human Provider／Upgrade Acceptanceはいずれも別Checkpointの対象です。

## Next 3 — Review Structured Output Contract Correction（PB-3bo.1／PB-3bo.3／PB-3bo.5／PB-3bpでclosure完了）

Codex PB-3bo診断が、Review Structured Output Schema（`review.TypedDecisionJSONSchema`）とGo parser（`review.ParseTypedDecision`）の間に意味論的契約の不一致（P1）を指摘しました。`summary` fieldはGo側が非空を厳格に要求する一方、Provider Schema側は`type:"string"`のみで非空を強制できておらず（Anthropic対応Schema subsetに`minLength`／`pattern`がないため）、Providerが仮に空文字列のsummaryを構造的に正しいStructured Outputとして生成した場合、verdict・issuesの判定内容自体は有効なReviewをGo側が丸ごと失敗させてしまう、という不一致でした。

PHASE PB-3bo.1で、`verdict`フィールドが既に実証していた`const`（closed value、Anthropic対応Schema subsetで利用可能）を使い、`anyOf`の各verdict別branchの`summary`をverdictごとに異なる固定文字列へ閉じることでこれを解消しました（`review.SummaryApprove`＝「レビューの結果、問題は見つかりませんでした。」、`review.SummaryRequestChanges`＝「レビューの結果、修正が必要な指摘があります。詳細は下記の指摘事項を参照してください。」）。Promptの出力例・ルール説明もこの2値と完全に同期しました。Go側の既存の厳格な非空検証（`ParseTypedDecision`）はPB-3bo.1時点では無変更のまま維持し、影響範囲は`go/internal/review/schema.go`・`go/internal/prompt/review.go`と対応する固定fixtureのみでした。

Codex PB-3bo.2診断のP1は、Schema側の`const`強制がGo境界（`review.ParseTypedDecision`）自体には反映されておらず、Structured Outputsを経由しない経路では任意の非空summaryや反対verdict用summaryがなお受理されうる、という残存ギャップを指摘しました。P2は、PB-3bo.1のROADMAP／ADR記述が「詳細な判定理由はissues[]にすべて保持されるためReview evidenceは失われない」と一般化していた点が、Approveについては不正確（Approve固有の自由記述の肯定的理由はfresh Reviewではもう収集されない）だったと指摘しました。PHASE PB-3bo.3で、`parseDecision(requireSummary=true)`へcanonical verdict別の完全一致チェックを追加し（前後空白付き・反対verdict用・任意の非空値をいずれも新しい閉じたReason `review.ParseFailureInvalidSummary`／`Field: "summary"`で拒否、空文字列・空白のみは既存Reasonのまま維持、`DecodeDecision`は無変更）、上記の一般化した記述を訂正しました（正確な区別はADR-0040「PB-3bo.3 Correction」節）。この一致判定を新たに要するようになったため、`go/internal/synthesisacceptance/scenario_v1.json`と`fixtures/provider/browser_acceptance_v1.json`（実daemon binaryを使うBrowser Gateが同じGo parserを通るため、23箇所）、および多数のGo test（process／service／httpapi／cmd/workcairnの各package、共有mock helperとgolden fixture）のsummaryも固定値へ同期しました。`review/result.go`本体以外の既存の失敗伝播・FailureEnvelope配線・bounded_acceptance profile契約（ADR-0072）・canonical Review JSON Contractのfield／type／永続化経路レベルの契約は無変更です。詳細はADR-0040「PB-3bo.3 Correction」節。

Codex PB-3bo.4診断のP0/P1は、`summary`はverdict別に閉じられていたものの、`issues`とverdictの組合せ自体は閉じられておらず、`{"verdict":"Approve","issues":[非空],"summary":"<Approveの固定値>"}`という組合せをGo側がなお受理しうる、という残存ギャップを指摘しました。PHASE PB-3bo.5で、`parseDecision(requireSummary=true)`へ「verdictがApproveならissuesは必ず空」という判定を追加し（issueの個別field読み取りより前に判定するため不正issue本文は一切検証・露出しない）、新しい閉じたReason `review.ParseFailureIssuesForbidden`／`Field: "issues"`で拒否するようにしました。これにより、fresh Reviewが到達しうる有効な組合せはApprove+空issues+`SummaryApprove`とRequest Changes+非空issues+`SummaryRequestChanges`の2つだけに閉じられました。Schema側（`TypedDecisionJSONSchema`）のApprove branchへ対応する空配列強制（`maxItems`等）は、そのキーワードの使用実績がリポジトリ内に一切なかったため、要件の「実証済みの場合だけ追加」という限定に従い意図的に追加していません——Go側のparser判定がこの組合せの唯一の機械的保証です。既存3 fixture（`browser_acceptance_v1.json`・`claude_review_typed_decision_request_v1.json`・`scenario_v1.json`）の全Review responseをtruth tableに照らして確認し、Approve+非空issuesの組合せは1件も存在しなかったため変更していません。副次的に、`ParseError.Field`／`Decision.Summary`／`ReviewExecutionResult.ParseFailureField`／`OrchestrationResult.ParseFailureField`のdocコメントと、Field設定条件を「missing_required_fieldのみ」と誤って限定していた古いtest名（`TestParseTypedDecisionParseErrorFieldOnlyForMissingRequiredField` → `TestParseTypedDecisionParseErrorFieldScopedToFieldSpecificReasons`）を、invalid_summary／approve_issues_forbiddenを含む正確な記述へ同期しました。詳細はADR-0040「PB-3bo.5 Correction」節。

`go test ./...`（focused packageは`-race`込み）・`gofmt`・`go vet`は全てPASSし、関連するBrowser/mock test（`conversation`／`bounded-acceptance`／`revision-limit-recovery`／`no-progress-recovery`／`budget-recovery`／`parallel-workflow`／`failure`の各spec）も実行し確認しました。Codex PB-3bo診断のP2（stop reasonの永続化）は引き続きscope外であり、着手していません。retry、fallback、Revision、Provider routing、Keychain、bounded profile契約（ADR-0072）はいずれも変更していません。本Correctionは、PB-3ag／PB-3jで観測されたProvider failure根本原因の修正でも、実Providerでの成功確認でも、Public Beta GOへの近接を示すものでもありません。commit／push／tag操作／candidate生成はいずれも行っていません。

PHASE PB-3bpで、上記P2だけを対象にした未commit実装を追加しました。Reviewの`max_tokens`先行拒否は従来どおりですが、`ReviewService`からprocess境界へ既存のProvider-neutral `worker.StopReasonMaxTokens`を型付きで運び、Review Commandの`failure.Envelope.Category=max_tokens`としてCommand Ledgerへ永続化します。`Provider`／`Substage`は空のままで、未知・欠落・偽造stop reasonはCategoryへ保存しません。canonical Review JSON／Markdown、Revision／Recovery、retry／fallback、Provider routing、UI、Keychain、bounded profile契約は変更していません。設計はADR-0077（`Proposed`）に記録し、focused review後にのみAcceptedへ移行します。
...
Phase U-6.1で、このread-only FeedをCompany Viewへ薄く投影するLocal Web UIをcommit `7e81e3947a766169f15d3577cb0baf5a62e0ede3`で実装済みです。browserはbackend順序を変更せず、closed type／actionを表示labelへ写し、Interaction項目だけを既存の依頼詳細へ開きます。Routine reconcile用の新HTTP mutationは追加せず、未知の将来値はliteral safe labelへ縮退します。empty state、section-local API failure、typed／unknown item、live clarification由来の`human_input_required`を`tests/browser/company-attention.spec.mjs`で確認しています。Attentionを正本化せず、ranking・自動実行・retry・新しい推測を持たないADR-0065の境界は不変です。
...
PHASE PB-2.29で最終tag候補commit（`fa05fc3`）へ`make public-beta-smoke`／`make check-ui-full`／`make v1-release-gate`を新規に各1回実行しGreenを確認し、PHASE PB-2.31でこのcommitをGitHub mainへpushし、PHASE PB-2.33でHuman承認のもとrepositoryをPublic化してPrivate Vulnerability Reportingを有効化しました。続くPB-3 Human Packaged-Binary／Provider Acceptanceで、Human Operator自身のMacでpackaged binary（darwin/arm64）のchecksum、native folder picker、Starter Organization、native Keychain経由の実Claude Provider接続までは成功しましたが、実Providerが生成した唯一のTaskのTitle／RationaleがともにJSON構文上は正当な非空文字列`"junk"`となり、既存`ceoplan.isPlaceholderValue`（ADR-0067）が完全一致`"placeholder"`だけを拒否する設計だったため素通りし、Human承認候補まで到達しました。Human Operatorは正しく承認を保留し、既定のno-retry方針に従って再実行せずAcceptanceを停止しました。PHASE PB-3dで、この実証済み非content token`"junk"`をADR-0067と同じ共有canonical境界（`ceoplan.NormalizeCandidate`）へ、完全一致集合の拡張という最小の形で追加し、ADR-0067を追記（過去の事実は変更せず）しました。PHASE PB-3eのCodex独立reviewでは、production validation本体は正しい一方、process／Apply test、archive履歴、wrapping-punctuation説明にP1 1件、P2 2件、P3 1件が報告されました。focused correction（PHASE PB-3d.1）でこれらを修正し、PHASE PB-3e.1のCodex focused re-reviewでこの4件すべての解消と新しいP0〜P3 findingなしを確認し、PB-3d／PB-3d.1／PB-3d.2の8ファイルをcommit可能と判定しました。

PHASE PB-3fでこの8ファイルを単一commit`38080fe`（`fix: reject known non-content plan tokens`）として確定しました（push未実施）。PHASE PB-3gの`38080fe`に対する最初のAutomated Gate実行では、`make public-beta-smoke`と`make check-ui-full`（69 passed／1 known skip）はPASSしましたが、`make v1-release-gate`は`TestRunnerSerializesCEOIntentRequestFixture`（`fixtures/provider/claude_ceo_intent_request_v1.json`）でFAILしました。production schemaへ追加した`junk`wordingがこのgolden fixtureへ未同期だったことが原因で、既知flakeとの一致はなく、retryせず停止し、source変更やfailureの隠蔽は行っていません。この差分だけを`fixtures/provider/claude_ceo_intent_request_v1.json`のnon-review description／review description／summary descriptionの3か所（3 insertions／3 deletions）へ反映し、単一commit`d233b2d`（`fix: sync CEO Intent request fixture with junk-token schema wording`、親`38080fe`）として確定しました（production source変更なし、fixture全体再生成なし、push未実施）。`d233b2d`に対するfresh Automated Gate実行では、`make public-beta-smoke`、`make check-ui-full`（69 passed／1 known skip）、`make v1-release-gate`（build matrix、`go test ./...`全package、`go test -race ./...`全package、`go vet`、`gofmt`）がすべてPASSしました。既知Go race flake、Browser timing flakeのいずれも発生せず、実Provider／credential／実Vaultはいずれも使用していません。

Public化（PHASE PB-2.33）後、PHASE PB-2.34でrepositoryをPublic化した状態に対する独立cross reviewを実施し、P0〜P3のfindingはありませんでした。PHASE PB-2.35で、PB-3d修正前のHEAD（`fa05fc3`）からmacOS／arm64のlocal candidate archive（`dist/final-fa05fc372ca9/workcairn_v1.0.0-beta.1_darwin_arm64.tar.gz`、SHA-256 `0e74a6d214a366f0ca3081249e0ff2375228f02730427d55cbfb1aebad68fcbe`）を生成・検証しました。このarchiveはPB-3d／PB-3d.1のsource修正を含まないため、そのままでは新しいRelease候補として再利用できません。

`d233b2d`はPHASE PB-3gのfresh Automated GatesをGreenで通過したsource／fixture候補です。このROADMAP同期自体はまだ未commitのため、ROADMAP同期を含む後続commitを次のtag対象候補とし、そのHEADでfinal Automated Gatesを新規実行してから確定します。GitHub mainへのpush、後続commit向けのrelease candidate archive生成、`v1.0.0-beta.1` tag作成、GitHub Release作成はいずれも未実施です。repository Public化とPrivate Vulnerability Reporting有効化（PHASE PB-2.33）はそれぞれ完了済みですが、**PB-3 Human Packaged-Binary／Provider Acceptanceの再実施とPublic Beta Releaseの承認は、いずれも本Checkpoint終了時点で未完了です。**

PHASE PB-3iで最終tag対象候補（`ec2940be`）からmacOS／arm64のnew final candidate archiveを生成・検証しました。続くPHASE PB-3j Human Packaged-Binary／Provider Acceptance再開で、packaged daemonを`--claude-credential-source keychain`で起動したところ、既存Keychain項目はmetadata上存在するにもかかわらずKeychain読み取りに失敗し、`credential_source_unavailable`でlistener開始前に停止しました。PHASE PB-3kのread-only診断（credential値・daemon再実行なし）で、新旧packaged daemon binaryがいずれもad-hoc署名でTeam IDを持たず、rebuildごとに異なるcode hash（designated requirement）を持つことを確認し、macOSのKeychain ACLが旧binaryの作成した項目への新binaryのアクセスを拒否している可能性を、証拠に基づく最有力仮説として特定しました（署名／ACLの根本原因自体は未確定）。PHASE PB-3lのCodex cross-reviewはこの診断を確認しました。この結果、**Public Betaは本Checkpoint終了時点でもNO-GOのままです。** PHASE PB-3mで、Local OS Adapterがすでに計算していたsecret-freeなcredential failure classification（substage／category）を、Runtime境界（`runtime.readCredential`）で失わずに保持する実装を追加し、packaged daemonの起動失敗を安全に診断しやすくしました。PHASE PB-3m.1のCodex reviewでP1 2件・P2 2件（独立substage／category mapによる不正な組合せの通過、任意文字列を設定できるexported field）を発見し、PHASE PB-3m.2でsource／substage／categoryを1つの有効startup tupleとして検証するprivate closed enumへ置き換え、exported raw diagnostic fieldを削除してread-only accessorだけを公開する形へfocused correctionしました。この5ファイルはPHASE PB-3m.4で単一commit`24e5a78`（`fix: preserve closed credential diagnostics`、親`ec2940be`）として確定しました（push未実施）。`headless-local` credentialによるPB-3 Acceptance再開は、Keychain経路自体の動作確認やRelease GO判定の代替にはなりません。既存のarchive／Automated Gate evidence（`fa05fc3`・`d233b2d`・`ec2940be`のいずれも）は、signing／Keychain問題の修正後に生成される新HEAD向けのFinal Releaseへ、そのまま再利用しません。

PHASE PB-3nで、PB-3kが特定した「ad-hoc署名・Team IDなし・rebuildごとに異なるdesignated requirement」というPacked binary Keychain ACL問題への恒久対策として、[ADR-0071](adr/ADR-0071-macos-developer-id-signing-and-notarization.md)を新設し、macOS Developer ID署名／notarization Architectureを設計・決定しました。Apple公式資料（Developer ID、Notarizing macOS software、Creating distribution-signed code、Packaging Mac software、Customizing the notarization workflow、Resolving common notarization issues）だけを根拠に、ad-hoc署名・unsigned binaryのrelease archive化を恒久禁止し、3 binaryへcanonical code-signing identifier（`com.workcairn.cli.workcairn`／`com.workcairn.cli.workcairn-daemon`／`com.workcairn.cli.workcairn-core`、既存Keychain service `com.workcairn.provider.anthropic`とは別namespace）、Hardened Runtime、secure timestampを必須化し、canonical macOS distribution containerをtar.gzからDMGへ移行（Apple notary serviceがtar.gzを受理しないため。PKGはinstall副作用のため不採用、ZIPは署名・直接stapleができないため不採用）、notarization credentialは`notarytool` Keychain profile経由の非secret参照だけをrelease scriptへ渡す方針を決定しました。実Team ID、実際に生成されるdesignated requirement、証明書renewal互換性、PB-3jの直接原因がACL mismatchだったか自体はいずれも未検証のまま「Unconfirmed hypotheses」として明記し、Apple Developer Program加入・証明書・notarization profileの存在はHuman prerequisiteとして一切仮定していません。本Checkpointは設計・文書化のみであり、`scripts/package-release.sh`／`scripts/verify-release-archive.sh`／Makefile／source／binary／archiveはいずれも変更せず、署名・notarization実装、実Apple account操作は一切行っていません。**Public Betaは本Checkpoint終了時点でもNO-GOのままであり、ADR-0071の実装とHuman prerequisiteの確認はいずれも後続Checkpointの対象です。**

PHASE PB-3n.1のCodex独立reviewでP1 5件・関連P2 findingsが報告され、PHASE PB-3n.2でADR-0071・Architecture.md・PublicReleaseChecklist.md・PublicBetaFirstRunAcceptance.md・本ROADMAPへのfocused correctionを行いました（Release順序をlocal tag作成先行へ修正、identity表示名一致からSHA-1／Team ID／notary profileの3変数contractへ変更、Section A単独のPublic Beta GO表現を削除しhistorical evidence明記、Gatekeeper検証をDMG層／3 CLI層の両方必須へ分離、Upgrade Acceptanceのbuild N／N+1 provenanceとcredential separationを明確化、Apple要件とWorkCairn product policyの記述分離、PB-3jのcausality表現をACL mismatch最有力仮説・direct causality未確定で統一、DMG固定contractとverifier mount contractを追加、notarization bounded wait timeoutを90分に固定、verifier evidenceを5分類化）。Developer ID／Hardened Runtime／notarization／DMGという基本方針自体は変更していません。**Public Betaは本Checkpoint終了時点でもNO-GOのままです。** Apple Developer Program加入、証明書、notarization profileというHuman prerequisiteはいずれも未確認のままであり、署名・notarization実装は`scripts/package-release.sh`／`scripts/verify-release-archive.sh`／Makefileのいずれにも未着手です。tag作成、GitHub Release作成はいずれも未実施のままです。現在のlocal HEAD `24e5a78`は、この署名・notarization実装が行われる**前**の状態（PB-3m.4のcredential diagnostic修正までのcommit）であり、本ROADMAP自体を含む現在のdocs差分（ADR-0071の新設と4文書の更新）はこのHEAD上でまだcommitされていない未確定の変更です。現時点をclean・final tag-target commitと同一視しないでください——`24e5a78`はPB-3n.1／PB-3n.2の検討対象であるドキュメント草案の親commitに過ぎず、release candidateとしての確定はまだ行われていません。署名・notarizationとPB-3jのKeychain ACL問題とのcausalityも、引き続き未確定のままです。

PHASE PB-3n.3のCodex focused reviewで、findingはP0 0件・P1 0件・**P2 2件**・P3 0件でした（P2-1: ADR-0071の証明書renewal／rotation説明が「更新してもSHA-1 fingerprintは変わらない」と読める不正確な記述だった。P2-2: notarization timeoutの実装例が標準macOS環境で保証されないGNU `timeout`へ依存していた）。PHASE PB-3n.4で、ADR-0071と本ROADMAPだけへfocused correctionを行いました——renewal／replacement／rotationでは通常新しい証明書・新しいSHA-1 fingerprintが発行されること、rotation時はHumanが`RELEASE_SIGNING_IDENTITY_SHA1`を明示的に更新すること、旧証明書・新証明書・別identityへのhidden fallbackを禁止すること、`RELEASE_EXPECTED_TEAM_ID`もrotationの前後を問わず毎回実証値と照合すること、rotationを跨いだupgrade acceptanceは実証されるまで未確認事項のままとすること、既存署名済みartifactの扱いと将来artifactの署名証明書選択を別問題として整理すること、をADR-0071「1」へ追加しました。notarization bounded wait（deadline 90分、方針自体は不変）の実装契約を、`notarytool`のnative timeout optionが実装時点で確認できればそれを使い、なければWorkCairnが所有するGo製release helper／process supervisorがGo標準libraryだけでdeadlineを実装する（GNU `timeout`／Homebrew版`timeout`／Python／Node等の非標準依存を持たない、Go Only方針に従う、deadline到達時は子processをcancel／terminate／必要ならkillし必ず`Wait`でreapする、submission IDをstrictに取得しdeadline時に未取得ならre-submitせず安全停止する）へADR-0071「7」を修正しました。この2文書以外（`docs/Architecture.md`／`docs/PublicBetaFirstRunAcceptance.md`／`docs/PublicReleaseChecklist.md`）はこのPHASEでは変更していません。Developer ID／Hardened Runtime／notarization／DMG／local tag先行／3変数identity contract／new-user・upgrade・Gatekeeper・Provider acceptanceという既存方針はいずれも変更していません。**Public Betaは本Checkpoint終了時点でもNO-GOのままです。** 署名・notarizationの実装（`scripts/package-release.sh`／`scripts/verify-release-archive.sh`／Makefile）は引き続き未着手で、Apple Developer Program加入・証明書・notarization profileというHuman prerequisiteも未確認のままです。現在のlocal HEAD `24e5a78`は`origin/main`へ**未push**のままであり、本PHASEのdocs差分（ADR-0071とROADMAPの更新）もこのHEAD上でまだcommitされていません。`v1.0.0-beta.1`のtag、GitHub Releaseはいずれも未作成です。既存のarchive／Automated Gate／PB-3 Acceptance evidence（`fa05fc3`・`d233b2d`・`ec2940be`のいずれも）は、署名・notarization実装後に生成する新しいrelease candidateへ引き続き再利用できません。次はCodexによる`PB-3n.5 Focused Architecture Re-review`を予定しています。

PHASE PB-3n.5のCodex focused reviewで、findingはP0 0件・**P1 1件**・**P2 2件**・P3 0件でした（P1: ADR-0071が既存署名済みartifactの有効性を、certificate revocationやnotarization ticket revocationを考慮せず無条件に断定していた。P2-1: 新証明書のfingerprintに関する「同じCSR・同じ鍵ペアを再利用しない限り」という例外の書き方が不正確だった。P2-2: Apple公式資料とlocal `notarytool 1.1.2 (41)`の両方でnative `--timeout`が確認できるにもかかわらず、native timeout optionとGo supervisorの二択を残していた）。PHASE PB-3n.5aで、ADR-0071と本ROADMAPだけへfocused correctionを行いました——新規発行された証明書は同じ鍵ペア・CSR・Team ID・display nameであってもfingerprintの継続を推測せず常に新しいfingerprintとして扱いHumanが`RELEASE_SIGNING_IDENTITY_SHA1`を毎回明示的に再取得・更新する契約へ修正し、certificate expiry（provisioning profile不使用のsoftwareは期限切れ後も継続動作しうる）・certificate renewal／rotation・certificate revocation（既にinstall済みでもinstall／起動不可になる）・notarization ticket revocation（署名鍵漏洩時等にAppleと協力してticket自体を無効化できる）の4つを区別し、既存artifactの有効性についてApple公式資料の範囲を超える保証をしない記述へ修正しました。notarization timeoutは、Go supervisor分岐を撤回し、`notarytool submit ... --wait --timeout <90分相当値>`というnative契約へ一本化しました（release preflightで`--wait`／`--timeout`のcapabilityを確認しfail-closed、GNU `timeout`／Homebrew／Python／Node等は不使用）。90分というclient側wait上限自体は、Apple側notarization job処理そのものを90分でcancelする保証ではなく、client timeout後もApple側submissionが継続し得ることを明記しました。`notarytool`がclient側timeout時にsubmission IDを常に返すかどうかは確定済みのApple公式契約として扱わず、help／versionからの確認・fake-tool test・実notarization実行時のsanitized evidenceで明らかにする未検証事項としました。この2文書以外（`docs/Architecture.md`／`docs/PublicBetaFirstRunAcceptance.md`／`docs/PublicReleaseChecklist.md`）はこのPHASEでは変更していません。Developer ID／Hardened Runtime／secure timestamp／SHA-1完全一致によるidentity選択／`RELEASE_EXPECTED_TEAM_ID`／`RELEASE_NOTARY_PROFILE`／3 CLI canonical identifier／DMG canonical container／local tag先行／tag push・GitHub Release別承認／DMG層と3 CLI層のGatekeeper検証／New-user・Upgrade・Gatekeeper・Provider acceptance／explicit approval before effects／no retry・no fallback／PB-3j causality未確定／headless-local非代替／90分wait deadlineという既存方針はいずれも変更していません。**Public Betaは本Checkpoint終了時点でもNO-GOのままです。** 現在のdocs差分（ADR-0071とROADMAPの更新）は未commitのままで、local HEAD `24e5a78`は`origin/main`へ引き続き未pushです。署名・notarization実装（`scripts/package-release.sh`／`scripts/verify-release-archive.sh`／Makefile）は引き続き未着手で、Apple Developer Program加入・証明書・notarization profileというHuman prerequisiteも未確認のままです。`v1.0.0-beta.1`のtag、GitHub Releaseはいずれも未作成です。既存のarchive／Automated Gate／PB-3 Acceptance evidence（`fa05fc3`・`d233b2d`・`ec2940be`のいずれも）は引き続き新しいrelease candidateへ再利用できません。次はCodexによる`PB-3n.5b Focused Architecture Re-review`を予定しています。

PHASE PB-3n.5bのCodex focused reviewで、findingはP0 0件・P1 0件・**P2 2件**・P3 0件でした（P2-1: ADR-0071のcanonical Release sequence step 9とPublic Release ChecklistのmacOS signing／notarization項目からmandatory `--timeout`が欠落していた。P2-2: `90m`がlocal `notarytool`のhelpで確認済みであるにもかかわらず、`1h30m`との選択を未確定のまま残していた）。PHASE PB-3n.5cで、`docs/adr/ADR-0071-macos-developer-id-signing-and-notarization.md`、`docs/PublicReleaseChecklist.md`、本ROADMAPだけへfocused correctionを行いました——canonical Release sequence step 9とSection 7のcanonical invocationを`notarytool submit ... --wait --timeout 90m`へ統一し、`90m`をcanonical durationとして固定し（`1h30m`は非documented formのまま）、Public Release ChecklistのmacOS signing／notarization項目も同じ`--wait --timeout 90m`表現へ同期しました。`notarytool`がclient側timeout時にsubmission IDを返すかどうかは、引き続き未確認のまま維持しています（help／versionからの確認・fake-tool test・実notarization実行時のsanitized evidenceで明らかにする別項目のまま）。`docs/Architecture.md`／`docs/PublicBetaFirstRunAcceptance.md`はこのPHASEでは変更していません。Developer ID／Hardened Runtime／secure timestamp／SHA-1完全一致identity selection／certificate rotation時の明示fingerprint再取得／expiry・rotation・certificate revocation・ticket revocationの区別／`RELEASE_EXPECTED_TEAM_ID`／`RELEASE_NOTARY_PROFILE`／3 CLI canonical identifier／DMG canonical container／local annotated tagのbuild前作成／tag push・GitHub Release別承認／DMG・3 CLIのGatekeeper二層検証／New-user・Upgrade・Gatekeeper・Provider Acceptance／explicit approval before effects／no retry・no fallback／PB-3j causality未確定／headless-local非代替という既存方針はいずれも変更していません。**Public Betaは本Checkpoint終了時点でもNO-GOのままです。** 現在のdocs差分は未commitのままで、local HEAD `24e5a78`は`origin/main`へ引き続き未pushです。署名・notarization実装は引き続き未着手で、Apple Developer Program加入・証明書・notarization profileというHuman prerequisiteも未確認のままです。`v1.0.0-beta.1`のtag、GitHub Releaseはいずれも未作成です。既存のarchive／Automated Gate／PB-3 Acceptance evidenceは引き続き新しいrelease candidateへ再利用できません。次はCodexによる`PB-3n.5d Final Architecture Re-review`を予定しています。

PHASE PB-3o.1系のplan-review loopで、ADR-0071実装の詳細設計（POSIX shell安全パターン、fake-tool test seam、canonical binary signing order等）を確定しました。PHASE PB-3o.2で、release inspector（`go/internal/releaseinspector`／`go/cmd/workcairn-release-inspector`）とtool boundary library（`scripts/lib/release_tools.sh`のfixed absolute tool-path bundle contract）というSlice 1の安全な基礎部品を実装し、Codex独立reviewでP1 3件・P2 1件の指摘（focused correction: PB-3o.2b）を経て、この2つの基礎部品を単一commit`1eae847`（`build: add release inspection safety core`）として確定しました。release inspectorはsource-onlyのrelease-engineering primitiveであり、正式3 binaryではなく、archive／DMGへ含まれず、production signing workflowへ未接続です。

PHASE PB-3o.3で、Developer ID署名・DMG・notarization・staple・検証・promotionの完全automation実装（Slice 2）を試み、独立review（PB-3o.3a）でrelease-safety findingsを報告されました。focused correction（PB-3o.3a.1）で対応しましたが、この完全automationは複雑性と追加findingsが残ったまま、**一度も本repositoryへcommitされませんでした**。PHASE PB-3o.3cで、この未commitの実装一式をrepository外のbounded backup（復元可能な形で退避、backup絶対pathはrepository文書へ記載しない）へ保存した上で、working treeをPB-3o.2 Slice 1 commit`1eae847`のclean状態へ復元し、release historyへ混入させませんでした。

PHASE PB-3pで、initial Public Beta releaseの方針を、未commitの完全automationではなく、[Manual macOS Signed Release Procedure](ManualMacOSReleaseProcedure.md)に沿ってHumanが1 stepずつApple標準commandを明示実行するbounded manual signed release pathへ切り替えました。完全automationは将来の`M-RELEASE-1`Checkpointへ延期します。この切替はDeveloper ID Application署名、Hardened Runtime、secure timestamp、Team ID一致、signed DMG、Apple notarization、staple、DMG／3 CLI Gatekeeper、exact content確認、checksum、New-user／Upgrade／Gatekeeper／Provider Human Acceptanceという既存の安全要件を一切弱めません——automationの有無はこれらAcceptance条件の代替にも免除事由にもなりません。PHASE PB-3p.1で、[ADR-0071](adr/ADR-0071-macos-developer-id-signing-and-notarization.md)、`docs/Architecture.md`、本ROADMAP、[Public Release Checklist](PublicReleaseChecklist.md)、[macOS First-run Acceptance](PublicBetaFirstRunAcceptance.md)、README英日、[Public Beta Quickstart](PublicBetaQuickstart.md)、[Operator Guide](OperatorGuide.md)をこのmanual pathへ同期し、[Manual macOS Signed Release Procedure](ManualMacOSReleaseProcedure.md)を新設しました。一般利用者向け配布asset案内も、初回Public Beta canonical assetとしてtar.gzからDMG（`workcairn_<version>_darwin_arm64.dmg`＋隣接`.sha256`）へ同期しました（Linux向けtar.gzは変更していません）。**Public Betaは本Checkpoint終了時点でもNO-GOのままです。** Apple Human prerequisite（Apple Developer Program加入、Developer ID証明書、`notarytool` profile）はいずれも未確認、signed candidateは未生成、notarizationは未実施、signed New-user／Upgrade／Gatekeeper／Provider Acceptanceはいずれも未実施、tag（`v1.0.0-beta.1`）／GitHub Releaseはいずれも未作成です。次の実施順序は、docs commit→final Automated Gates再実行→main push承認→Apple Human prerequisites確認→local annotated tag承認→[Manual macOS Signed Release Procedure](ManualMacOSReleaseProcedure.md)によるsigned candidate生成→Human Acceptance→tag push承認→GitHub Release承認、の順です。次はCodexによる`PB-3p.2 Manual Release Documentation Review`を予定しています。

PHASE PB-3u.1aで、tagged candidateの署名開始前read-only確認により、[Manual macOS Signed Release Procedure](ManualMacOSReleaseProcedure.md)§10の3 binary署名commandに`--force`が欠落していることが判明しました。`go build`が生成する各binaryはGo linkerによる既存の`adhoc,linker-signed`署名を既に持ち、`codesign`は既存署名の置換に`--force`を要求するため、このままでは署名処理が失敗する手順不足でした。署名は未開始のまま停止し、該当3 commandだけへ`--force`を追加するfocused correctionを行いました（`--deep`は追加せず、既存署名置換の根拠がないDMG署名commandへも`--force`を追加していません）。
