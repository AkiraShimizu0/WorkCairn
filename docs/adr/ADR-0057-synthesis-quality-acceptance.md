...
## Addendum: Prompt Quality v2, StopReason, and the Human Review Artifact (Checkpoint follow-up)

A real Claude baseline against this scenario (`claude-sonnet-5`) passed at 10/12: Cross-Evidence Synthesis and Actionability scored 1/2 each, every other item scored full marks. This is a targeted follow-up to that specific gap, not a rubric or threshold change.

- The general-purpose Task Prompt (`internal/prompt/builder.go`, shared by every Task, not a Synthesis-specific Prompt type) gained one additional System section, gated on `len(DependencyEvidence) > 1` -- i.e. only when a Task genuinely has more than one direct dependency to relate (the shape every fan-in Synthesis Task has). It asks the model to relate two or more pieces of evidence into one conclusion instead of summarizing each independently, and to give its top priority a concrete action, its evidence basis, an expected outcome, and a way to validate it, without inventing numbers the evidence does not contain. A Task with zero or exactly one dependency (including every ordinary Revision continuation) is byte-for-byte unaffected -- proven by the existing golden-fixture and single-dependency tests passing unmodified. The wording is scenario-neutral; it hardcodes no scenario-specific term and is verified not to.
- `worker.StopReason` (`internal/worker/runner.go`) is a new Provider-neutral enum (`completed`/`max_tokens`/`stop_sequence`/unknown) the Claude Adapter derives from its own raw `stop_reason` field, threaded additively through `worker.RunResult` → `worker.ExecutionResult` → `execution.Result` the same way `Usage`/`Duration` already are. It is observational only in this Checkpoint: no production dispatch decision reads it. `synthesisacceptance.Result.OutputTruncated` is `true` only when StopReason is exactly `max_tokens`, never inferred from the output token count.
- The Human Review Artifact (`synthesisacceptance.Config.ArtifactPath`, `-artifact-path` / `make synthesis-acceptance ARTIFACT_PATH=...`) is an opt-in-only file capturing the canonical Synthesis Deliverable's full text plus the same safe metadata as the existing report, so a human can review actual output that a temporary Vault's cleanup previously discarded. Never written unless explicitly configured; the caller chooses a path outside the Git working tree and any real Vault; never contains a credential, Authorization header, or raw Provider request.
- Evidence Coverage, Conflict Handling, Prioritization, and Unsupported Claims -- the four items that already scored full marks -- were not touched. The rubric, its pass threshold (10/12), and the deterministic keyword-group evaluation strategy Decision item 3 already committed to are unchanged.
