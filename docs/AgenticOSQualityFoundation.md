...
**PHASE PQ-1 addendum — placeholder value is now a runtime contract, not just an Evaluator score.** The "placeholder" Summary noted in row B above recurred across multiple real Provider runs (T-3, T-8, T-13) and, most recently, a Human Acceptance run whose Plan Summary and a Task title were both the literal string `"placeholder"`. This is a Planning *output-quality* defect, not an Evaluator gap — `internal/planningacceptance`'s 5-axis scoring already reflected it (T-8 scored poorly in part because of it) but scoring is not the same as preventing the value from reaching a Human Operator. PHASE PQ-1 added a narrow, exact-match runtime guard in `ceoplan.NormalizeCandidate` — the single boundary every Plan (Interaction-, Responsibility-, and Routine-generated, plus Apply-time re-validation) already passes through — that rejects a Plan Summary, Task Title, or Task Rationale that is exactly the token `"placeholder"` (case-insensitive, trimmed), returning a typed `ParseFailurePlaceholderValue` failure. There is no retry, fallback, or content substitution: a rejected Plan stays rejected until a Human Operator explicitly starts a new Planning request. See [ADR-0067](adr/ADR-0067-planning-placeholder-rejection.md) for the full root-cause analysis and design. This does **not** re-open or extend the 5-axis Evaluator itself, and it does not claim to solve Planning content quality in general — only this one evidence-backed literal.
