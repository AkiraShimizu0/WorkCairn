...
| B. Planning Quality | Foundation built (Phase Q), Fake-Provider-only | `internal/planningacceptance` now scores Intent Coverage, Dependency Quality (structural graph-shape comparison), Unsupported Assumptions, and Missing Information Awareness against a real generated CEO Plan via `process.GenerateCEOPlan` — see `docs/PlanningQualityAcceptance.md`. No Real Provider run yet, no pass threshold yet, Decomposition/Prioritization/Execution-Readiness quality still unscored. The Synthesis-layer `Prioritization`/`Conflict Handling` rubrics remain Synthesis-scoped only and do not measure CEO Plan-layer decomposition. |
