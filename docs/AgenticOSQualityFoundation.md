...
| B. Planning Quality | 5 real Provider runs (Phase T, T-3, T-4, T-8; Phase T-7 stopped at credential preflight), Work Coverage axis added Phase T-10 | `internal/planningacceptance` scores Intent Coverage, Work Coverage (Task-only text, added Phase T-10 after real runs T-3/T-8 scored identically despite opposite Human Review — see `docs/PlanningQualityAcceptance.md`), Dependency Quality (structural graph-shape comparison), Unsupported Assumptions, and Missing Information Awareness against a real generated CEO Plan via `process.GenerateCEOPlan`. No pass threshold yet; full Decomposition granularity/Prioritization/Execution-Readiness quality still unscored; Dependency Quality's own task-count-vs-graph-reasoning conflation (Phase T-9) remains open. The Synthesis-layer `Prioritization`/`Conflict Handling` rubrics remain Synthesis-scoped only and do not measure CEO Plan-layer decomposition. |
