...
| B. Planning Quality | 5 real Provider runs (Phase T, T-3, T-4, T-8; Phase T-7 stopped at credential preflight), Work Coverage axis added Phase T-10, Dependency Quality common-prefix refinement added Phase T-12 | `internal/planningacceptance` scores Intent Coverage, Work Coverage (Task-only text, added Phase T-10 after real runs T-3/T-8 scored identically despite opposite Human Review), Dependency Quality (common-prefix structural graph-shape comparison, refined Phase T-12: a task-count mismatch is now a diagnostic Detail rather than an automatic 0, full credit requires the entire expected graph was reachable for comparison — T-3/T-8 no longer share the same ambiguous 0), Unsupported Assumptions, and Missing Information Awareness against a real generated CEO Plan via `process.GenerateCEOPlan` — see `docs/PlanningQualityAcceptance.md`. No pass threshold yet; full Decomposition granularity/Prioritization/Execution-Readiness quality still unscored; concept-based node mapping and required-edge modeling remain deferred (Phase T-11). The Synthesis-layer `Prioritization`/`Conflict Handling` rubrics remain Synthesis-scoped only and do not measure CEO Plan-layer decomposition. |
