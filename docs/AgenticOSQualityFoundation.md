...
| B. Planning Quality | Minimal | `Prioritization`/`Conflict Handling` rubrics test Synthesis-layer priority and trade-off *language* only. CEO Plan-layer goal decomposition and dependency judgment are not scored at all — the Task graph in the scenario is fixture-wired by the harness, not generated and graded. |
