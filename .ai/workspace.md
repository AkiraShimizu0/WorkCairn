...
- ADR-0074（Accepted）のRecovery Plan Previewは、既存plannerを変更操作なしで1回だけ呼び、Task state／Version／Action／Executable／closed blockerだけをsafeなHTTP viewとLocal Web UIへ表示します。Preview responseやbrowser stateはApply authorityではなく、Recovery apply、自動修復、retry、artifact adoptionは未実装です。ADR-0073／0074の機能はGitHub mainへ反映済みですが、公開済み`v1.0.0-beta.1`のtag対象commitより後の変更であり、既存candidate／Releaseへは含まれません。次回配布には新version／tag／candidate／Acceptanceが必要です。
