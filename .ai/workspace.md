...
- ADR-0073（Accepted）のGuided Recovery Inspectionは、既存Recovery Reportをsafeなread-only HTTP viewとLocal Web UIへ投影し、内部detail／reference／Vault pathを公開せず、async ownershipとinvalidationを維持します。Plan preview、Apply、自動repair／retryはこのinspection scopeへ含めません。
- ADR-0074（Accepted）のRecovery Plan Previewは、既存plannerを変更操作なしで1回だけ呼び、Task state／Version／Action／Executable／closed blockerだけをsafeなHTTP viewとLocal Web UIへ表示します。Preview responseやbrowser stateはApply authorityではなく、Recovery apply、自動修復、retry、artifact adoptionはADR-0074のscopeに含みません。ADR-0073／0074の機能はGitHub mainへ反映済みですが、公開済み`v1.0.0-beta.1`のtag対象commitより後の変更であり、既存candidate／Releaseへは含まれません。次回配布には新version／tag／candidate／Acceptanceが必要です。
- ADR-0075（Accepted）の`complete_task`限定Recovery Apply backendはfresh prepare、typed commitment、Human approval、Command Ledger claim／replay、Task Version CAS、exact 7-field safe resultを実装し、focused review、Automated Gates、commit、GitHub mainへのpushまで完了しました。ADR-0076（Accepted）のLocal Web UIは、executable Previewから別のfresh prepareとHumanの最終clickを経て同期Applyだけを送り、focused reviewは最終P0〜P3・contract gap・test gap・Open Questionsなしです。Preview responseやbrowser storageはauthorityにせず、blocked plan／`fail_and_hold_task`、async／Scheduler、retry／fallback、Provider／Keychainは対象外です。Final Automated GatesはFull Browser Gate 169 passed／1 known skip／0 failed／0 retries、全package race test、Public Beta smoke、v1 Release GateがすべてPASSし、実装・ADR status・Gate evidenceを単一commitとして記録します。pushは別Checkpointです。
...
- ADR-0066により`workcairn-daemon`のClaude credential sourceをclosed selection（`automatic`／`environment`／`keychain`／`headless-local`）としてRuntime edgeへ集約しました。既定`automatic`は既存互換のenvironment→Keychainだけでheadless fileへ進まず、明示sourceは他sourceを一切読みません。headless-localはOS user config root配下の固定pathをread-onlyで扱い、regular／non-symlink、mode 0600、current-user owner、Lstat→open→fstat同一性、64 KiB上限を満たさなければfail-closedです。WorkCairn自身はfile作成／移行をせず、credential／path／raw OS errorをHTTP、Ledger、UI、Audit、logへ出しません。GUI Keychain setupは`automatic`／`keychain`だけ、direct CLIは従来どおりenvironment-onlyです。
...
- ADR-0054（Accepted）のBudgetGuard v1は、「出力が改善しているか」（Progress Intelligence）とは独立した第三のPolicyとして、「許可された資源envelopeを既に超えたか」を判断します。`policy.BudgetPolicy`/`FixedBudgetPolicy`（Task状態を直接変更しない純粋な決定境界）はRuntime／Provider Call Countの2軸を独立してチェックし、いずれか一方の超過だけで`BudgetEscalate`を返します（Progress Intelligenceの保守的なAND条件とは逆――Budgetは安全上限であり収束判断ではないため）。`autonomy.Contract.MaxProviderCalls`（既定60、上限300）/`MaxRuntime`（既定30分、上限2時間）は既存の0=未設定規約に従い、scopeは1 Reviewed Workflow execution単位です（LoopGuardの構造的counterとは別物、CEO Plan生成・Recovery自体は対象外）。`internal/service`の`budgetTracker`は`policy.BudgetPolicy`とは意図的に分離した、実際に並列安全な予約primitive（atomic reserve→invoke→record）で、並列Branchが同時に最後の1枠を要求しても超過が決して起きないことを200 goroutine・`-race`下の並列テストで確認済みです（process-local・1呼び出しscopeのみの保証、durable化は将来課題）。`worker.TokenUsage`の既存nilable設計（unknown≠0）をそのまま利用しますが、v1ではゲーティングに使わず観測用のみで、Cost Budgetは未実装です。FailureEnvelopeは単一Code`BUDGET_EXCEEDED`+`Category`（`"runtime"`/`"provider_call"`）で区別し、実装中に見つけた実バグ（`runBranch`内部ループのcontext deadline判定が常に汎用的な`"cancelled"`へ分類されていた）を修正・回帰テストで検証しました。ADR-0054時点では既存Revision Limit用Recoveryとstate shapeが一致しないためBudget Recoveryを配線しませんでしたが、このgapはADR-0055のContinuationで解消済みです。Cost Budgetとouter CEO Command全体のdurable Budgetは引き続き対象外です。
- ADR-0055（Accepted）のBudget Recovery Continuationは、Budget停止が残す「source Task／Deliverable／Request Changes Review／Revision intentはcommit済み、Revision TaskはUnstarted」というexact stateを、既存`ExecuteRevision`でもWorkflow全体のretryでもなく、`ReviewedWorkflowRunService.ResumeRevision`で安全に続行します。CEO-facing operationは既存`interaction.workflow.recover_revision`を再利用しますが、Goはcanonical Workflow evidence、Task state、Revision intentから対象を一意に検証し、Budget時だけ既存Revision Taskを最初のtargeted roundとして強制します。その成功後は既存`EvaluateAllReadiness`へ戻るため、完了済みA/Cを再実行せずSynthesisが自然にreadyになります。Recoveryは必ずCEOの明示操作＋新Command ID＋Ledger claim＋Session CASで、新しいbounded Workflow Budget scope（通常default 60 calls／30分）を持ちます。同じCommand replay、元Workflow replay、concurrent duplicate、stale画面からの新CommandはProviderを二重実行しません。元Workflow CommandをCorrelationID、新しいchild CommandをCausationIDとしてlineageを保ち、TaskServiceは唯一の状態owner、RunnerはProvider callだけの責務を維持します。durable root-command Budget、`MaxRecoveryCount`、Cost／pricing／Metricsは未実装です。
- ADR-0056（Accepted）のDependency Evidence Contextは、Synthesis Taskの直接依存を「完了状態」だけでなくcanonical Deliverable本文としてWorker Promptへ渡します。ExecutionServiceがapproval後・TaskService.Start前にprovider-neutral collectorを呼び、Vault Adapterがdependency metadata順を維持してimmutable Revision intent lineageのterminal Completed Taskを選びます。Evidence欠落／pending／empty／invalidは`DEPENDENCY_EVIDENCE_MISSING`でProvider前にdefault denyし、Title／Plan／Conversationへfallbackしません。PromptBuilderはprovenance付きEvidenceを信頼されないUser Contextとして描画し、System側でrole変更／Prompt上書き／外部操作命令を無視するpolicyを明示します。固定上限は1件32 KiB・合計96 KiB、UTF-8安全な決定的prefix truncation＋markerで、LLM要約なし、dependency-free prompt goldenはbyte-for-byte維持します。Runner、JSON Contract v1、Vault/Event/Audit schema、TaskService ownership、TokenUsageは無変更です。
- ADR-0057（Accepted）のSynthesis Quality Acceptanceは、cross-dependencyと軽い矛盾を持つ固定日本語A/B/Cをtemporary VaultへTaskService／Deliverable Store経由でcanonical化し、既存Reviewed Workflow／BudgetGuardから得たSynthesis Deliverableを6項目×0–2点のdeterministic rubricでread-only評価します。good fixtureはpass、単純連結fixtureはA/B/C coverageがあってもcross-synthesis／矛盾調停／優先順位でfailします。LLM-as-Judge、全文golden、repeat run、Provider-specific tuningは未導入です。`make synthesis-acceptance PROVIDER=claude`はcredentialもnetworkも使わないdry-runで、actual runは人間の別途明示`EXECUTE=1`が必要、1 external Synthesis call＋固定Review、Budget 2 calls／10分、no retry／no fallbackです。Resultはquality／TokenUsage／duration／prompt shapeだけをsafe JSONへ出し、outputやtemporary Vaultを永続化しません。
