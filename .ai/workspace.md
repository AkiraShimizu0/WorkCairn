...
- ADR-0036によりdaemonは起動時Provider設定をnetwork accessなしのredacted statusとして公開し、未接続時はPlan生成前に案内します。新規Web Interactionは依頼ごとのModel選択を要求せず論理`workcairn-auto`を使います。Role／Task別routingは既存Runner Registry手前の将来typed policyであり、未接続Providerへのfallbackはしません。
- Local Web UIの`AI Connections`はClaudeのredacted接続状態とAutomatic routingを表示するread-only Settingsです。trusted LANからsecretを入力せず、将来の永続接続はMac loopback限定SettingsからRuntime edgeのCredential Store Adapterを通してmacOS Keychainへ保存する方針を第一候補とします。
